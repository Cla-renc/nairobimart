...
export default async function AdminDashboard() {
    try {
        ...
        validOrders.forEach(order => {
            ...
            if (order.items && Array.isArray(order.items)) {
                order.items.forEach(item => {
                    ...
                    orderCost += safeCostPrice * (item.quantity || 1);
                    orderBaseRevenue += safeUnitPrice * (item.quantity || 1);
                });
            }
        ...
        const ordersForChart = validOrders.filter(o => o.createdAt && new Date(o.createdAt) >= sevenDaysAgo);
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const dailyRevenue = ordersForChart
                .filter(o => o.createdAt && new Date(o.createdAt) >= dayStart && new Date(o.createdAt) < dayEnd)
            ...
            <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <Card className="border-none shadow-sm">
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">KES {Number(totalRevenue || 0).toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</div>
                    ...
                    <Card className="border-none shadow-sm">
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-green-600">KES {Number(totalProfit || 0).toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</div>
                    ...
                    <Card className="border-none shadow-sm bg-yellow-50/50">
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-yellow-700">{Number(totalReferrals || 0).toLocaleString()}</div>
                    ...
                    <Card className="border-none shadow-sm">
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{Number(totalOrders || 0).toLocaleString()}</div>
                    ...
                    <Card className="border-none shadow-sm">
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{Number(totalCustomers || 0).toLocaleString()}</div>
                                    ...
                                    {recentOrdersDb.map((order) => (
                                        <div key={order.id} className="flex items-center justify-between">
                                            ...
                                            <div className="text-right">
                                                <p className="text-sm font-bold leading-none">KES {Number(order.total || 0).toLocaleString()}</p>
                                                <Badge className={cn(
        ...
        return (
            <div className="p-8 bg-red-50 border border-red-200 rounded-lg">
                <h2 className="text-red-700 font-bold text-xl mb-4">Admin Dashboard Error</h2>
                <p className="text-red-600 mb-4">An error occurred while loading the dashboard data. This is often caused by invalid data in the database.</p>
                <div className="bg-white p-4 rounded border overflow-auto text-sm text-gray-800">
